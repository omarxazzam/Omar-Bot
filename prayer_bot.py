...
app = Flask('')
@app.route('/')
def home(): return "<b>Omar Smart Bot V21.0 (Fixed) is Online! 🚀</b>"
...
def get_next_prayer_info():
    ...
    for key, p_time in today_prayers:
        if p_time > now:
            ...
            return f"⏳ **الصلاة القادمة:** {prayer_names[key]}\n⏱️ **متبقي:** {h} ساعة و {m} دقيقة"
    fajr = today_prayers[0][1] + timedelta(days=1)
    diff = fajr - now
    h, m = diff.seconds // 3600, (diff.seconds % 3600) // 60
...
def start_auto_reminders():
    ...
    def remind_dhikr():
        while True:
            time.sleep(2400)
            msg = random.choice(["لا إله إلا الله", "سبحان الله وبحمده", "أستغفر الله العظيم"])
            for user in users_collection.find({}):
...
def rec_prayer(c):
    ...
    bot.edit_message_text(f"✅ تم تسجيل {p_name}", c.message.chat.id, c.message.message_id)

# --- 6. الدالة الذكية (تم إصلاحها لعدم قص الرسائل) ---
@bot.message_handler(func=lambda m: True)
def handle_all(m):
    text = m.text
    cid = m.chat.id
    
    if text == "⏳ كم باقي على الصلاة؟": bot.reply_to(m, get_next_prayer_info(), parse_mode="Markdown")
        ...
        if t:
            msg = "🕌 **المواقيت (اليوم):**\n"
            for k in ['Fajr','Dhuhr','Asr','Maghrib','Isha']: msg += f"🔹 {k}: {convert_to_12h(t[k])}\n"
            bot.reply_to(m, msg)
    elif text == "📊 تقريري اليومي": bot.reply_to(m, get_today_report(cid), parse_mode="Markdown")
    else:
        # AI Response (بدون Markdown لتجنب الأخطاء)
        if model:
            try:
                bot.send_chat_action(cid, 'typing')
                prompt = f"أنت مساعد إسلامي ذكي ومؤدب باللهجة المصرية. ساعد هذا المستخدم: {text}"
                response = model.generate_content(prompt)
                # 🛑 التغيير هنا: حذفنا parse_mode="Markdown" عشان الرسالة توصل كاملة
                bot.reply_to(m, response.text) 
            except Exception as e:
                bot.reply_to(m, "⚠️ حدث خطأ بسيط، حاول مرة أخرى.")
                print(f"AI Error: {e}")
        else:

...
# تأكد من أن هذا الرابط هو الصحيح الذي يعمل معك
...
# رسائل تظهر لو كان الأداء جيداً (للتثبيت)
GOOD_MSGS = [
...
# رسائل تظهر لو كان الأداء ضعيفاً (للتذكير والمحاسبة)
BAD_MSGS = [
...
# --- 3. بيانات الأذكار (كاملة + عدد مخفف) ---
MORNING_ADHKAR = [
    {
    ...
    {
        "text": "سُبْحَانَ اللهِ وَبِحَمْدِهِ. (يكفي 10 مرات وأكمل الباقي بقلبك)",
        "count": 10
    },
    {
...
EVENING_ADHKAR = [
    {
        "text": "💎 **آية الكرسي:**\n{اللّهُ لاَ إِلَـهَ إِلاَّ هُوَ الْحَيُّ الْقَيُّومُ...} (اقرأ الآية كاملة)",
        "count": 1
    },
    {
...
app = Flask('')
@app.route('/')
def home(): return "<b>Omar Smart Bot V9.0 (Quranic Edition) is Online! 🚀</b>"
...
def get_today_report(chat_id):
    ...
    # حساب النسبة (5 صلوات أساسية)
    total_required = 5
    ratio = done_count / total_required
    
    report_msg += "\n➖➖➖➖➖➖\n"
    
    # اختيار الرسالة (تشجيع أو محاسبة)
    if ratio >= 0.6: # لو صلى أكثر من 60%
...
def main_menu():
    ...
    markup.add(types.KeyboardButton("📊 تقريري اليومي")) # تم تغيير الاسم من إحصائياتي
    return markup

@bot.message_handler(commands=['start'])
def start(message):
    register_user(message.chat.id, message.from_user.first_name)
    bot.send_message(message.chat.id, f"أهلاً بك يا **{message.from_user.first_name}** 👋\n\nتم تحديث البوت: الأذكار كاملة، ونظام المحاسبة الجديد يعمل! ✅", reply_markup=main_menu(), parse_mode="Markdown")
